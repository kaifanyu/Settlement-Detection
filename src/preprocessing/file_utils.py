...
def process_viirs_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    viirs = file_path.name.split('_',)[2]
    viirs  = viirs.split('.')[0]
    date = datetime.strptime(viirs[1:],"%Y%j").strftime("%Y-%m-%d")
    return (date, '0')
...
def process_s1_filename(file_path: Path) -> Tuple[str, str]:
    ...
    Tuple[str, str]
        A tuple containing the date and band.
    """
    s1 = file_path.name.split("_")[3:]
    date = datetime.strptime(s1[0],"%Y%m%d").strftime("%Y-%m-%d")
    band = s1[1].split(".")[0]
    return (date,band)
...
def process_s2_filename(file_path: Path) -> Tuple[str, str]:
    ...
    Tuple[str, str]
    """
    s2 = file_path.name.split("_")[1:]
    date = datetime.strptime(s2[0],"%Y%m%d").strftime("%Y-%m-%d")
    band = s2[1].split(".")[0]
    band = band[1:]
    return (date,band)
...
def process_landsat_filename(file_path: Path) -> Tuple[str, str]:
    ...
    Tuple[str, str]
        A tuple containing the date and band.
    """
    landsat = file_path.name.split("_")[2:]
    date = datetime.strptime(landsat[0],"%Y-%m-%d").strftime("%Y-%m-%d")
    band = landsat[1].split(".")[0]
    band = band[1:]
    return (date,band)
...
def get_satellite_files(tile_dir: Path, satellite_type: SatelliteType) -> List[Path]:
    ...
    for file in tile_dir.iterdir():
        if (pattern in str(file)) and (file.suffix == ".tif") and (not file.is_dir()):
            valid = True
        else:
            valid = False
        if valid:
            path_objects.append(Path(file))
    return path_objects
...
def get_unique_dates_and_bands(
    ...
    unique_bands_set = set()
    
    #Get all the dates and band to add to sets
    for file in satellite_files:
        date_and_band = grouping_function(file)
        date = date_and_band[0]
        band = date_and_band[1]
        unique_dates_set.add(date)
        unique_bands_set.add(band)
    
    #Convert sets to sorted lists to return
    unique_dates_list = list(unique_dates_set)
    unique_bands_list = list(unique_bands_set)
    unique_dates_list.sort()
    unique_bands_list.sort()
    
    return (unique_dates_list,unique_bands_list)
...
def load_satellite(tile_dir: Path, satellite_type: SatelliteType) -> xr.DataArray:
    ...
    stacked_data = np.stack(date_dimension)
    ...
    data_array = xr.DataArray(stacked_data, dims=("date","band","height","width"),coords={
        "date":dates,"band":bands,"height":range(stacked_data.shape[2]),"width":range(stacked_data.shape[3])
    })
...
def load_satellite_list(
    ...
    for satellite_type in satellite_type_list:
            satellite = load_satellite(tile_dir,satellite_type)
