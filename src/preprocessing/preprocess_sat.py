...
def minmax_scale(data_array: xr.DataArray, group_by_time=True) -> xr.DataArray:
    ...
    if group_by_time == True:
        ...
        for band in range(data_array.shape[1]):
            ...
            img_min = np.min(minimums)
            img_max = np.max(maximums)
            ...
            for date in range(data_array.shape[0]):
                ...
                # else
                    ...
                    data_array[date][band] = (data_array[date][band].values-img_min)/(img_max-img_min)
...
def convert_data_to_db(data_array: xr.DataArray) -> xr.DataArray:
    ...
    data_array.values = np.where(data_array.values == 0,np.nan,data_array.values)
    data_array.values = np.log10(data_array.values)
...
def maxprojection_viirs(data_array: xr.DataArray) -> xr.DataArray:
    ...
    reshaped_maximum = np.reshape(maximum,(1,1,800,800))
    max_viirs_array = xr.DataArray(reshaped_maximum,dims=("date","band","height","width"),coords={
        "date":range(1),"band":range(1),"height":range(800),"width":range(800)
    })
...
def preprocess_sentinel1(
    ...
    """
    # convert data to db
    sentinel1_data_array = convert_data_to_db(sentinel1_data_array)
    
    # quantile clip (make sure to pass the parameter)
    sentinel1_data_array = quantile_clip(sentinel1_data_array,clip_quantile,True)
    
    # apply a gaussian_filter (make sure to pass the parameter)
    sentinel1_data_array = gaussian_filter(sentinel1_data_array,sigma)
    
    # minmax and return
    return minmax_scale(sentinel1_data_array,True) 
...
def preprocess_sentinel2(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    sentinel2_data_array = quantile_clip(sentinel2_data_array,clip_quantile,True)
    
    # gamma correct (make sure to pass the parameter)
    sentinel2_data_array = gammacorr(sentinel2_data_array,gamma)
    
    # minmax and return
    return minmax_scale(sentinel2_data_array,True)
...
def preprocess_landsat(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    landsat_data_array = quantile_clip(landsat_data_array, clip_quantile,True)
    
    # gamma correct (make sure to pass the parameter)
    landsat_data_array = gammacorr(landsat_data_array,gamma)
    
    # minmax and return
    return minmax_scale(landsat_data_array,True)
...
def preprocess_viirs(
    ...
    """
    # quantile clip (make sure to pass the parameter)
    viirs_data_array = quantile_clip(viirs_data_array,clip_quantile,True)
    
    # minmax and return
    return minmax_scale(viirs_data_array,True)