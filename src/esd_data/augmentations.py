...
class Blur(object):
    ...
    def __call__(self, sample: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        blur_lambda = lambda img: cv2.blur(img,ksize)
        img = apply_per_band(img,blur_lambda)
...
class AddNoise(object):
    ...
    def __call__(self, sample):
        ...
        random_sd = np.random.uniform(0,self.std_lim)
        ...
        noise = np.random.normal(self.mean,random_sd,(img.shape[-2],img.shape[-1]))
        
        # add the noise to the img and clip between 0 and 1
        addition_lambda = lambda img: np.add(img,noise)
        clip_lambda = lambda img: np.clip(img,0,1)
        img = apply_per_band(img,addition_lambda)
        img = apply_per_band(img,clip_lambda)
...
class RandomVFlip(object):
    ...
    def __call__(self, sample):
        ...
        if (random.random() <= self.p):
            ...
            v_flip_lambda = lambda img: cv2.flip(img,0)
            img = apply_per_band(img,v_flip_lambda)
...
class RandomHFlip(object):
    ...
    def __call__(self, sample):
        ...
        if (random.random() <= self.p):
            # apply per band the cv2.flip function (you can pass it as a lambda)
            h_flip_lambda = lambda img: cv2.flip(img,1)
            img = apply_per_band(img,h_flip_lambda)
