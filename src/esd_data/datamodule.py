...
class ESDDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.exists() or not(any(self.processed_dir.iterdir())):
            ...
            tile_dirs = [tile_dir for tile_dir in self.raw_dir.glob('*') if tile_dir.is_dir()]
            ...
            for train_tile_dir in tqdm(list(tile_dirs_train),desc="Processing train tiles"):
                # get the data array list and gt data array from load_and_preprocess
                data_array_list,gt_data_array = self.load_and_preprocess(train_tile_dir)
            ...
            for validation_tile_dir in tqdm(list(tile_dirs_val),desc="Processing validation tiles"):
                # get the data array list and gt data array from load_and_preprocess
                data_array_list,gt_data_array = self.load_and_preprocess(validation_tile_dir)
